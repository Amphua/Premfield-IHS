@echo off
echo Running database migration for sports house and CCA fields...
echo.
...
if not exist "database\migration_add_sports_cca.sql" (
    echo ERROR: Migration file not found: database\migration_add_sports_cca.sql
    pause
    exit /b 1
)
...
psql -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -f "database\migration_add_sports_cca.sql"

if %ERRORLEVEL% EQU 0 (
    echo.
    echo SUCCESS: Migration completed successfully!
    echo Sports house and CCA fields have been added to the students table.
) else (
    echo.
